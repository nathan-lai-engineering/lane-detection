...
class LoadImages:  # for inference
    def __init__(self, path, detect_fps, img_size=640, save_csv=None):
        ...
        videos = [x for x in files if os.path.splitext(x)[-1].lower() in vid_formats and (save_csv is None or not (os.path.basename(x) + ".csv") in os.listdir(save_csv))]
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            curr_fps = math.ceil(self.cap.get(cv2.CAP_PROP_FPS))
            if self.detect_fps:
              inc = int(max(curr_fps / self.detect_fps, 1))
            else:
              inc = 1
            ret_val = None
